...
class ModuleAttempt extends RepositoryAbstract
{
    /**
    * @var Monolog\Logger
    */
    ...
    public function findForEnrollmentModule($enrollmentId, $moduleId, $params = [])
    {
        $dql = "
            SELECT ma
            FROM Hondros\Api\Model\Entity\ModuleAttempt ma
            WHERE ma.enrollment = {$enrollmentId} AND ma.module = {$moduleId} ";
        
        if (!empty($params['type'])) {
            $dql .= "AND ma.type = '{$params['type']}' ";
        }
        
        $dql .= "ORDER BY ma.created DESC";
