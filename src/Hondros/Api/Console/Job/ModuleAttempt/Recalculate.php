...
namespace Hondros\Api\Console\Job\ModuleAttempt;
...
class Recalculate extends Command
{
     ...
     */
    public function getModuleAttemptService()
    {
        return $this->getServiceManager()->get('moduleAttemptService');
    }
    ...
    {
        $this->setName("moduleAttempt:recalculate")
            ->setDescription("Updates information for any module attempt id in the queue");
    }

    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $startTime = time();
        $ttl = $this->getServiceManager()->get('config')->crontab->moduleAttempt->recalculate->ttl;
        $maxConsumers = $this->getServiceManager()->get('config')->crontab->moduleAttempt->recalculate->consumers;
        $processId = getmypid();

        $channel = $this->getMessageQueue()->channel();
        list(,,$consumerCount) = $channel->queue_declare(self::QUEUE_RECALCULATE_MODULE_ATTEMPT, false, false, false, false);
        ...
        $callback = function($msg) use ($output) {
            ...
            //$output->writeln("Trying to update module attempt id {$item->moduleAttemptId}");
            $results = $this->getModuleAttemptService()->recalculateBasedOnModuleAttemptQuestions($item->moduleAttemptId);
            //$output->writeln("Updated module attempt {$results['id']}");
        ...
        $channel->basic_consume(self::QUEUE_RECALCULATE_MODULE_ATTEMPT, '', false, false, false, false, $callback);
